 ...
 */
	...
	static int SHTPS_EVENT_POLL_INTERVAL = 			18;
	static int SHTPS_POLL_MIN_INTERVAL =			18000;
	static int SHTPS_POLL_STANDARD_VAL =			20000;
	#else
	...
	#if defined( SHTPS_NOTIFY_INTERVAL_GUARANTEED )
	#define SHTPS_EVENT_POLL_INTERVAL				18
	#define SHTPS_POLL_MIN_INTERVAL					18000
	#define SHTPS_POLL_STANDARD_VAL					20000
	#else
...
static int shtps_get_tm_rxsize(struct shtps_rmi_spi *ts)
{
	...
	return (receive_num > SHTPS_TM_RXNUM_MAX)? SHTPS_TM_RXNUM_MAX : receive_num;
}

static int shtps_get_tm_txsize(struct shtps_rmi_spi *ts)
{
	int trans_num   = F05_QUERY_NUMOFTRANSEL(ts->map.fn05.query.data);
	return (trans_num > SHTPS_TM_TXNUM_MAX)? SHTPS_TM_TXNUM_MAX : trans_num;
}
	...
	if(ts->diag.tm_mode == SHTPS_FWTESTMODE_V01){
		...
		if(mode == SHTPS_TMMODE_FRAMELINE){
			for(i = 0;i < trans_num;i++){
				...
				shtps_rmi_read(ts, 0x0202, &ts->diag.tm_data[i * receive_num], receive_num);
			}
		}else if(mode == SHTPS_TMMODE_BASELINE){
			for(i = 0;i < trans_num;i++){
				shtps_rmi_write(ts, 0x0201, (0x40 | (i & 0x3f)));
				shtps_rmi_read(ts, 0x0202, &ts->diag.tm_data[i * (receive_num * 2)], receive_num * 2);
			}
		}
		mutex_unlock(&shtps_ctrl_lock);
	}else{
		if(mode == SHTPS_TMMODE_FRAMELINE){
			...
			for(i = 0;i < trans_num;i++){
				...
				shtps_rmi_read(ts, ts->map.fn05.dataBase + 2, &ts->diag.tm_data[i * receive_num], receive_num);
			}
			mutex_unlock(&shtps_ctrl_lock);
		}else if(mode == SHTPS_TMMODE_BASELINE){
			mutex_lock(&shtps_ctrl_lock);
			...
			for(i = 0;i < trans_num;i++){
				...
				shtps_rmi_read(ts, ts->map.fn05.dataBase + 2, &ts->diag.tm_data[i * (receive_num * 2)], receive_num * 2);
			}
			mutex_unlock(&shtps_ctrl_lock);
		}
	}
