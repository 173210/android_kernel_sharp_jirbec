...
/* SHARP_EXTEND Modify check_size for receive data 2010.11.30 Mod-Start */
//		if (RMNET_IS_MODE_IP(opmode) ? (sz > dev->mtu) :
//						(sz > (dev->mtu + ETH_HLEN))) {

		if (RMNET_IS_MODE_IP(opmode) ? (sz > 1500) :
						(sz > (1500 + ETH_HLEN))) {
/* SHARP_EXTEND Modify check_size for receive data Mod-End */
