...
#define DEFAULT_FREQ_THRESHOLD 1024000
static unsigned int freq_threshold;

#define DEFAULT_SUSPEND_SPEED 368640
static unsigned int suspend_speed;

#define DEFAULT_RESUME_SPEED 1024000
static unsigned int resume_speed;
...
//	newtarget = freq_threshold;
	...
	for_each_cpu(cpu, tmp_mask) {
		if (!suspended) {
			if (target_freq == policy->max || target_freq == freq_threshold) {
				if (nr_running() == 1) {
...
//				__cpufreq_driver_target(policy, newtarget, CPUFREQ_RELATION_H);
			...
			if (target_freq > suspend_speed) {
				__cpufreq_driver_target(policy, suspend_speed, CPUFREQ_RELATION_H);
			} else {
				target_freq = cpufreq_interactivex_calc_freq(cpu);
...
static ssize_t show_suspend_speed(struct kobject *kobj,
				struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", suspend_speed);
}

static ssize_t store_suspend_speed(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
...
static struct global_attr suspend_speed_attr = __ATTR(suspend_speed, 0644,
		show_suspend_speed, store_suspend_speed);

static ssize_t show_resume_speed(struct kobject *kobj,
				struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", resume_speed);
}

static ssize_t store_resume_speed(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
...
static struct global_attr resume_speed_attr = __ATTR(resume_speed, 0644,
		show_resume_speed, store_resume_speed);

static ssize_t show_freq_threshold(struct kobject *kobj,
				struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", freq_threshold);
}

static ssize_t store_freq_threshold(struct kobject *kobj,
			struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input >= 0)
		freq_threshold = input;
	return res;
}

static struct global_attr freq_threshold_attr = __ATTR(freq_threshold, 0644,
		show_freq_threshold, store_freq_threshold);

static struct attribute *interactivex_attributes[] = {
	&min_sample_time_attr.attr,
	&suspend_speed_attr.attr,
	&resume_speed_attr.attr,
	&freq_threshold_attr.attr,
	NULL,
};
...
static void interactivex_suspend(int suspend)
{
	...
	if (!enabled) return;
		...
		suspended = 1;
                __cpufreq_driver_target(policy, suspend_speed, CPUFREQ_RELATION_L);
...
static int __init cpufreq_interactivex_init(void)
{
	...
	suspend_speed = DEFAULT_SUSPEND_SPEED;
	resume_speed = DEFAULT_RESUME_SPEED;
